...
def enlarge_border(path_points, enlargement_meters):
    path_points = np.array(path_points)
    if path_points.shape[0] < 3:
        return path_points  # Cannot enlarge a line or point
    enlarged_points = []
    num_points = len(path_points)
    for i in range(num_points):
        p1 = path_points[i]
        p2 = path_points[(i + 1) % num_points]  # Next point (wraps around)
        p0 = path_points[(i - 1) % num_points]  # Previous point (wraps around)
        # Calculate normalized vectors for the segments
        v1 = p2 - p1
        v1_norm = v1 / np.linalg.norm(v1)
        v0 = p1 - p0
        v0_norm = v0 / np.linalg.norm(v0)
        # Calculate the normal vector (perpendicular to the bisector)
        normal = v1_norm + v0_norm
        normal_norm = normal / np.linalg.norm(normal)
        # Calculate the enlarged point
        enlarged_point = p1 + normal_norm * enlargement_meters
...
borber_points_en = enlarge_border(border_points,Border_Margin)
