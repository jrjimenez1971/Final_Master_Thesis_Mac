...
def plot_results(wt_x_orig, wt_y_orig, final_sol, gens, gens_best, aep_list, circular_border_params_initial, title_suffix=""):
    ...
    # --- Verificación inicial de longitudes de coordenadas originales ---
    if len(wt_x_orig) != len(wt_y_orig):
        error_message = f"Error en plot_results ({title_suffix}): Las longitudes de wt_x_orig ({len(wt_x_orig)}) y wt_y_orig ({len(wt_y_orig)}) no coinciden."
        print(error_message)
        return  # Salir de la función plot_results
    print(f"plot_results ({title_suffix}): Longitudes iniciales de wt_x_orig y wt_y_orig coinciden ({len(wt_x_orig)}).")
    ...
    print(f"\n--- Plot 1 Debug ({title_suffix}) ---")
    print(f"Length of wt_x_orig: {len(wt_x_orig)}")
    print(f"Length of wt_y_orig: {len(wt_y_orig)}")
    ...
    for i, s in enumerate(gens_best):
        if isinstance(s, (tuple, list)) and len(s) == 2 and isinstance(s[0], (list, np.ndarray)) and isinstance(s[1], (list, np.ndarray)):
            print(f"  Generation {gens[i]}: Length of s[0] (x): {len(s[0])}, Length of s[1] (y): {len(s[1])}")
            plt.plot(s[0], s[1], 'y.', label=f'Generation {gens[i]} Layout' if i == 0 else "")
        else:
            print(f"Warning: Skipping plot for generation {gens[i]} due to unexpected data format: {s}")
    if final_sol is not None:
        print(f"Length of final_sol[0] (x): {len(final_sol[0])}")
        print(f"Length of final_sol[1] (y): {len(final_sol[1])}")
        plt.plot(final_sol[0], final_sol[1], 'g.', label='Final Layout')
    print(f"Length of np.array(wt_x_orig): {len(np.array(wt_x_orig))}")
    print(f"Length of np.array(wt_y_orig): {len(np.array(wt_y_orig))}")
    plt.scatter(np.array(wt_x_orig), np.array(wt_y_orig), c='r', s=10, label="Border Points (Original)")
    ...
    plt.close()
    filename = f"original_vs_genetic_layouts{title_suffix.replace(' ', '_')}.png"
    ...
    plt.close()
    filename = f"initial_vs_final_layouts{title_suffix.replace(' ', '_')}.png"
    filepath = os.path.join(output_dir, filename)
    print(f"Plot saved to: {filepath}")
    ...
    print(f"\n--- Plot 3 Debug ({title_suffix}) ---")
    print(f"Length of gens: {len(gens)}")
    print(f"Length of aep_list: {len(aep_list)}")
    plt.scatter(gens, aep_list, label='AEP Data Points')
    # (The lengths of x_trend and y_trend should be fine if gens and aep_list have data)
    plt.plot(np.linspace(min(gens) if gens else 0, max(gens) if gens else 1, 100), np.poly1d(np.polyfit(gens, aep_list, 2) if len(gens) > 2 else [0, 0, 0])(np.linspace(min(gens) if gens else 0, max(gens) if gens else 1, 100)), 'r-', label='Parabolic Trend Line')
    ...
    plt.close()
    filename = f"aep_evolution{title_suffix.replace(' ', '_')}.png"
    filepath = os.path.join(output_dir, filename)
    print(f"Plot saved to: {filepath}")

    # Plot 4 - Delta AEP evolution
    delta = np.diff([aep_ref] + aep_list)
    if gens and len(gens) > 1 and len(delta) == len(gens):
        plt.figure(figsize=(10, 6))
        plt.title(f'Evolution of AEP improvements per generation {title_suffix}')
        print(f"\n--- Plot 4 Debug ({title_suffix}) ---")
        print(f"Length of gens: {len(gens)}")
        print(f"Length of delta: {len(delta)}")
        plt.scatter(gens, delta, label='Delta AEP Data Points')
        plt.plot(np.linspace(min(gens), max(gens), 100), np.poly1d(np.polyfit(gens, delta, 2))(np.linspace(min(gens), max(gens), 100)), 'r-', label='Parabolic Trend Line')
        plt.xlabel('Generation')
        ...
        plt.close()
        filename = f"delta_aep_evolution{title_suffix.replace(' ', '_')}.png"
        filepath = os.path.join(output_dir, filename)
        print(f"Plot saved to: {filepath}")
    else:
