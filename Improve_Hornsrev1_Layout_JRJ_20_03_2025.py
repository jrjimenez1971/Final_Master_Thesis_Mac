...
def list_randon_values_one_by_one(lst1,lst2,Length,border):
    lst = lst1,lst2
    lst_rnd = lst1,lst2
    lst1_rnd = []
    lst2_rnd = []
    ...
    for i in range(n):
        lst1_rnd = lst1
        lst2_rnd = lst2
        while fitness(lst_rnd) <= fitness(lst):
            good_lst_rnd_val = False
            sx, sy = [lst1[i],lst2[i]]
            while good_lst_rnd_val == False:
                lst1_rnd_val = sx + (Length * rnd.uniform(-1,1))
                ...
                if filter_points_inside_border(border, new_point) == True:
                    good_lst_rnd_val = True              
            lst1_rnd[i] = lst1_rnd_val
            lst2_rnd[i] = lst2_rnd_val
...
def getSolutions():
    ...
    for x in iter_lengths.keys():
        ...
        for s in range(Iter_Num):
            x_sol,y_sol = list_randon_values_one_by_one(wt_x,wt_y,Iter_Length,mesh_border)
        ...
        for x in iter_lengths.keys():
            ...
            for _ in range(Iter_Num):
                ...
                for s in NewGen:
                    ...
                    new_gen_x_rnd = []
                    new_gen_y_rnd = []
                    new_gen_x_rnd,new_gen_y_rnd = list_randon_values_one_by_one(new_gen_x,new_gen_y,Iter_Length,mesh_border)
                x_sol = new_gen_x_rnd
                y_sol = new_gen_y_rnd                
